...
import { createJumpTower, createTetrisTower, createJumpMapArea, updateJumpmap, onWaxStep, onKeycapStep, onKeycapWalk, onPuddingStep, resetWax, isWaxStarted, refreshTetrisRanking } from './jumpmap.js';
import { openTetris } from './tetris.js';
...
// 테트리스 타워: 왁뿌 타워 동쪽 옆. 포탈은 맵 전환이 아니라 테트리스 오버레이(tetris.js) 실행.
const TETRIS_TOWER_POS = { x: 40, z: -13 };
const tetrisTower = createTetrisTower(TETRIS_TOWER_POS);
scene.add(tetrisTower.group);
...
(function setupMealPlan() {
    ...
    // iframe(임베드) 안내 배너 — MS 로그인 페이지는 iframe 표시를 차단하므로(X-Frame-Options)
    // 로그인 창이 뜨거나 X/빈 화면이면 창 안에서 해결이 불가능하다 → 새 탭 로그인 → 재시도로 유도.
    const hint = document.createElement('div');
    hint.style.cssText = 'padding:9px 16px; font-size:12px; color:#ffd9a0; background:#231a0e; border-top:1px solid #000; text-align:center; display:none; line-height:1.9;';
    const hintTxt = document.createElement('span');
    hintTxt.textContent = '🔐 로그인 창·X 표시·빈 화면이 뜨면 이 창 안에서는 MS365 로그인이 안 됩니다(보안 정책) → ';
    const hintBtnCss = 'background:#0364B8; color:#fff; border:none; border-radius:6px; padding:4px 10px; font-size:12px; cursor:pointer; font-family:inherit; margin:0 3px;';
    const hintOpen = document.createElement('button');
    hintOpen.textContent = '① 새 탭에서 로그인';
    hintOpen.style.cssText = hintBtnCss;
    hintOpen.onclick = () => { if (sourceUrl) window.open(sourceUrl, '_blank', 'noopener'); };
    const hintRetry = document.createElement('button');
    hintRetry.textContent = '② 로그인 후 다시 보기';
    hintRetry.style.cssText = hintBtnCss.replace('#0364B8', '#2E7D32');
    hintRetry.onclick = () => embed();
    hint.append(hintTxt, hintOpen, hintRetry);
...
let iceCreamGroup = null;    // 가게 전체 그룹 — 점프맵 입장 시 통째로 숨기기 위함
...
(function buildBaskinRobbins() {
    iceCreamGroup = new THREE.Group();
    _origSceneAdd(iceCreamGroup);   // 그룹은 원점(0,0,0) — 자식은 월드 좌표 그대로
    const iceAdd = (m, x, y, z) => { m.position.set(x, y, z); iceCreamGroup.add(m); return m; };
...
const COYOTE_MS      = 120;  // 가장자리 이탈(낙하 시작) 후에도 점프를 인정하는 유예 시간
const JUMP_BUFFER_MS = 150;  // 착지 직전에 누른 SPACE를 보관했다가 착지 즉시 점프하는 유예 시간
let _coyoteUntil = 0;        // 이 시각(performance.now)까지 낙하 중 점프 허용 — 가장자리 이탈 시 갱신
let _jumpBufferedUntil = 0;  // 이 시각까지 유효한 '착지 대기' 점프 입력 — 공중 SPACE 시 갱신
...
function updateSelfVertical(delta, moving) {
    ...
    if (_airborne) {
        ...
        if (_jumpVelY <= 0 && ny <= landY) {        // 하강 중 표면 도달 → 착지
            ...
            _coyoteUntil = 0;                       // 착지 완료 → 낙하 점프 유예 무효화
        }
        ...
        // 착지 직전(JUMP_BUFFER_MS 이내)에 눌러둔 SPACE 소비 → 착지 프레임에 즉시 재점프.
        // 실패 판정으로 리스폰됐다면 리스폰 쪽에서 버퍼가 초기화되므로 여기서 발동하지 않는다.
        if (justLanded && !_airborne && _jumpBufferedUntil && performance.now() <= _jumpBufferedUntil) {
            _jumpBufferedUntil = 0;
            _jumpVelY = JUMP_SPEED; _airborne = true;
        }
    } else {
        // ── 지상: 걷는 표면에 스냅 ──
        ...
        if (inJumpmap && hits.length) {                 // 걸어 올라선 발판(1번 등) 왁스도 깨짐 · 푸딩은 눌린 채 유지(둘 다 no-op 저렴)
            onWaxStep(hits[0].object);
            onPuddingStep(hits[0].object);
            onKeycapWalk(hits[0].object);               // 키보드 위 걷기 — 밟는 키마다 클릭음+백라이트
        }
        if (target > feetY + 0.02) {
            // 낮은 단차(≤STEP_UP) → 걸어 올라섬
            av.group.position.y = target;
        } else if (target < feetY - 0.06) {
            // 발밑이 꺼짐(가장자리 이탈) → 낙하 시작 + 코요테 타임 개시(직후 SPACE도 점프 인정)
            _airborne = true; _jumpVelY = 0;
            _coyoteUntil = performance.now() + COYOTE_MS;
        } else {
...
/** SPACE 점프 트리거 — 지상(또는 코요테 유예 중) 즉시 점프, 그 외 공중이면 착지 대비 버퍼링. 본인 아바타 필요. */
function trySelfJump() {
    ...
    if (_airborne) {
        // 가장자리 이탈 직후(코요테 타임)면 공중이라도 점프 인정 — 발판 끝 러닝 점프 씹힘 방지
        if (_coyoteUntil && performance.now() <= _coyoteUntil) {
            _coyoteUntil = 0;
            _jumpVelY = JUMP_SPEED;
            return;
        }
        // 그 외 공중 SPACE는 버퍼에 보관 → 착지 순간 자동 점프(더블점프는 여전히 불가)
        _jumpBufferedUntil = performance.now() + JUMP_BUFFER_MS;
        return;
    }
    _coyoteUntil = 0; _jumpBufferedUntil = 0;
    _jumpVelY = JUMP_SPEED;
    _airborne = true;
}
...
function resetSelfPosition() {
    ...
    _airborne = false; _jumpVelY = 0; _coyoteUntil = 0; _jumpBufferedUntil = 0;               // 점프/낙하 상태 해제
...
function animate() {
    ...
    // ── 내 캐릭터 키보드 이동 (WASD / 화살표) — 2D 모드에서는 map2d가 이동 전담 ──
    if (!window.__map2dActive && myPersonId && keysDown.size > 0 && !_isTyping()) {
        const av = personAvatarMap.get(myPersonId);
        if (av) {
    ...
    if (personAvatarMap.size > 0) {
        ...
        for (const [id, av] of personAvatarMap) {
            ...
            if (av.group.visible === false) continue;                   // 성능: 숨김(오프라인/1인칭) 아바타는 보간·애니 skip
            ...
            // 성능: 걷는 중 + 정지 후 0.6s(idle 복귀 정착)만 애니메이션. 정지·정착 후엔 본 계산 skip.
            if (isWalking) av._animUntil = elapsed + 0.6;
            if (isWalking || (av._animUntil && elapsed < av._animUntil)) {
    ...
    // 2D 탑다운 모드·테트리스 오버레이: 3D 렌더·라벨·홀로그램 스킵(map2d는 자체 캔버스, 테트리스는 불투명 오버레이가
    // 화면을 덮어 밑이 안 보임 → 마지막 프레임 정지화면 유지). WS 동기화·아바타 보간은 위에서 계속 동작.
    // 테트리스는 프레임 예산 확보 목적 — 3D 렌더가 프레임을 잡아먹으면 게임 키 반응(다음 화면 갱신)이 그만큼 늦어진다.
    if (!window.__map2dActive && !window.__tetrisActive) {
...
let _portalAction = null;      // 반경 안 포탈의 동작: 'jumpmap' | 'tetris' | 'exit'
 ...
 * 성능: 매 프레임 대량 호출되므로 clone() 제거하고 공용 벡터 재사용(GC 압력 제거).
 * onScreen: 카메라 뒤(z>=1)이거나 화면 밖이면 false → 호출부가 DOM 갱신을 건너뛰게 한다.
 */
const _w2sVec = new THREE.Vector3();
function worldToScreen(worldPos) {
    _w2sVec.copy(worldPos).project(camera);
    const onScreen = _w2sVec.z < 1 && _w2sVec.x >= -1.3 && _w2sVec.x <= 1.3 && _w2sVec.y >= -1.3 && _w2sVec.y <= 1.3;
    return {
        x: (_w2sVec.x * 0.5 + 0.5) * renderer.domElement.clientWidth,
        y: (-(_w2sVec.y * 0.5) + 0.5) * renderer.domElement.clientHeight,
        onScreen,
    };
}
...
/**
 * 경량 위치 델타(people-pos) 적용 — syncPersonAvatars의 위치 갱신 규칙과 동일.
 * 자기 아바타는 즉시 반영(단, 내가 키 조작 중이면 로컬 우선 skip), 원격은 targetPos만 갱신(보간).
 * 부서 고정 아바타는 데스크 위치 유지(위치 미반영) — 기존 동작과 일치.
 */
function _applyPeoplePos(id, position) {
    const av = personAvatarMap.get(id);
    if (!av || av.department || !position || position.x === undefined || position.y === undefined) return;
    const s = personPosToScene(position);
    if (id === myPersonId) {
        if (keysDown.size > 0) return;   // 내가 조작 중이면 로컬 조작 우선
        av.group.position.set(s.x, av.group.position.y, s.z);
    } else if (av !== draggingAvatar) {
...
function syncPersonAvatars(people) {
    ...
    for (const person of people) {
        ...
        if (person.position && (person.position.x !== undefined) && (person.position.y !== undefined)
            ...
            // 점프맵(별도 좌표계 z≈500)·2D 맵(로컬 이동 우선) 중에는 서버 에코가 내 아바타를
            // 저장된 오피스 좌표로 하드 스냅하지 않게 차단 — 점프맵 입장 직후 스냅백되어
            // "건물만 사라지고 못 들어가는" 버그의 원인이었다.
            const isSelfDetached = person.id === myPersonId && (inJumpmap || window.__map2dActive);
            if (av !== draggingAvatar && !isMovingByKey && !isSelfDetached) {
                const s = personPosToScene(person.position);
...
// 성능: (1) clone() 제거 — 공용 벡터 재사용, (2) 화면 밖 아바타는 오버레이 숨김+DOM 갱신 skip,
//       (3) 화면 좌표가 직전 프레임과 ~같으면(1px 미만) DOM 쓰기 skip(정지 아바타 리플로우 제거).
const _lblVec = new THREE.Vector3();
function _setOverlayPos(el, x, y, cacheKey, av) {
    // 정수 픽셀로 반올림 후 직전값과 동일하면 style 쓰기 생략(레이아웃 리플로우 방지).
    const ix = Math.round(x), iy = Math.round(y);
    const cx = cacheKey + 'X', cy = cacheKey + 'Y';
    if (av[cx] === ix && av[cy] === iy) return;
    av[cx] = ix; av[cy] = iy;
    el.style.left = `${ix}px`;
    el.style.top  = `${iy}px`;
}
function updatePersonLabels() {
    for (const [, av] of personAvatarMap) {
        const hidden = (av.labelEl && av.labelEl.style.display === 'none') || av.group.visible === false;
        // 화면 좌표 계산(공용 벡터 재사용). 머리 위 라벨 기준으로 온스크린 판정.
        _lblVec.copy(av.group.position); _lblVec.y += 1.9;
        const s = worldToScreen(_lblVec);
        const off = hidden || !s.onScreen;
        // 화면 밖/숨김 → 오버레이 전부 숨기고 DOM 위치 갱신 skip.
        if (off) {
            if (av.labelEl && av.labelEl.style.visibility !== 'hidden') av.labelEl.style.visibility = 'hidden';
            if (av.btnsEl && av.btnsEl.style.display !== 'none') av.btnsEl.style.display = 'none';
            if (av.bubbleEl && av.bubbleEl.style.visibility !== 'hidden') av.bubbleEl.style.visibility = 'hidden';
            if (av.meetingEl && av.meetingEl.style.visibility !== 'hidden') av.meetingEl.style.visibility = 'hidden';
            continue;
        }
        if (av.labelEl.style.visibility === 'hidden') av.labelEl.style.visibility = '';
        _setOverlayPos(av.labelEl, s.x, s.y, '_lbl', av);

        // 머리 위 버튼(🤖 AI / 👤 정보) — 이름 라벨 위쪽.
        if (av.btnsEl) {
            av.btnsEl.style.display = 'flex';
            _lblVec.copy(av.group.position); _lblVec.y += 2.45;
            const bs = worldToScreen(_lblVec);
            _setOverlayPos(av.btnsEl, bs.x, bs.y, '_btn', av);
        }
        if (av.bubbleEl) {
            av.bubbleEl.style.visibility = '';
            _lblVec.copy(av.group.position); _lblVec.y += 2.3;
            const bs = worldToScreen(_lblVec);
            _setOverlayPos(av.bubbleEl, bs.x, bs.y, '_bub', av);
        }
        // P6: 📹 인디케이터를 말풍선보다 위에 배치
        if (av.meetingEl) {
            av.meetingEl.style.visibility = '';
            _lblVec.copy(av.group.position); _lblVec.y += 2.9;
            const ms = worldToScreen(_lblVec);
            _setOverlayPos(av.meetingEl, ms.x, ms.y, '_mtg', av);
        }
    }
}
...
function updateAvatarKeyboardMove(delta, elapsed) {
    if (window.__map2dActive || !keyboardMoveEnabled || !selectedAvatarId || pressedMoveKeys.size === 0) return;
    const av = personAvatarMap.get(selectedAvatarId);
...
window.addEventListener('keydown', (e) => {
    if (window.__map2dActive) return;   // 2D 모드: 3D 전용 단축키(M·Enter 앉기/엘리베이터) 비활성
    if (window.__tetrisActive) return;  // 테트리스 오버레이 중: 게임이 키를 전담
...
function connectWS() {
    ...
    _ws.onmessage = (e) => {
        ...
        } else if (d.type === 'tetris-ranking') {
            refreshTetrisRanking(d.ranking);   // 누군가 최고 점수 갱신 → 타워 순위판 실시간 반영
        } else if (d.type === 'people-pos') {
            _applyPeoplePos(d.id, d.position);   // 경량 위치 델타(실시간 이동)
        } else if (d.type === 'dev-reload') {
            // 개발 편의(로컬 전용): 코드 수정 완료 신호(POST /api/dev/reload) — 목표 위치를 저장하고 새로고침.
            // localhost로 접속한 브라우저만 반응(배포 환경 사용자에게는 no-op). 서버 쪽도 루프백 전용.
            // 로드 후 _applyDevGoto(모듈 끝)가 아바타 준비를 기다렸다가 그 위치로 이동시킨다.
            if (location.hostname === 'localhost' || location.hostname === '127.0.0.1') {
                if (d.goto) { try { sessionStorage.setItem('dev.goto', JSON.stringify(d.goto)); } catch { /* noop */ } }
                location.reload();
            }
...
(function createViewSwitcher() {
    ...
    OPTS.forEach(({ view, label, title }) => {
        ...
        b.onclick = () => { if (window.__map2d && window.__map2d.isActive()) window.__map2d.exit(); setAvatarViewMode(view); };
        wrap.appendChild(b);
    });
...
window.__jmDebug = {
    ...
    exit: () => exitJumpmap(),
    isWaxStarted,
    ids: () => Array.from(personAvatarMap.keys()),   // 디버그: 등록된 아바타 id 목록
};

// ---- 2D용 캐릭터 스프라이트: 실제 3D 아바타 모델을 정면 렌더해 캔버스로 캐시 ----
// 수박·부리부리몬·펭귄·상세 휴먼 등 3D 외형 그대로 2D 맵에 표시된다.
const _sprCache = new Map();   // id → { key(외형 지문), canvas }
let _sprScene = null, _sprCam = null;
function getAvatarSprite(id) {
    const av = personAvatarMap.get(id);
    if (!av) return null;
    const key = `${av.displayName || ''}|${av.shirtColorHex || ''}|${av.avatarType || ''}`;
    const hit = _sprCache.get(id);
    if (hit && hit.key === key) return hit.canvas;
    try {
        if (!_sprScene) {
            _sprScene = new THREE.Scene();
            _sprScene.add(new THREE.AmbientLight(0xffffff, 1.1));
            const dl = new THREE.DirectionalLight(0xffffff, 1.6);
            dl.position.set(1.5, 3, 4);
            _sprScene.add(dl);
            _sprCam = new THREE.PerspectiveCamera(33, 96 / 128, 0.1, 20);
            _sprCam.position.set(0, 1.05, 3.4);
            _sprCam.lookAt(0, 0.95, 0);
        }
        const src = (av.personObj && av.personObj.group) || av.group;
        const clone = src.clone(true);
        clone.position.set(0, 0, 0);
        clone.rotation.set(0, 0, 0);
        clone.traverse((o) => { o.visible = true; });   // fpv 숨김 등 상태 초기화
        _sprScene.add(clone);
        const W = 96, H = 128;
        ...
        renderer.render(_sprScene, _sprCam);
        renderer.setRenderTarget(null);
        renderer.setClearColor(prevColor, prevAlpha);
        _sprScene.remove(clone);
        ...
        _sprCache.set(id, { key, canvas });
        return canvas;
    } catch {
        return null;   // 실패 시 map2d가 원형 폴백으로 그림
    }
}

// 아바타 스냅샷 목록 — 2D 맵(map2d)에서 사용
function getAvatarList() {
    const out = [];
    for (const [id, av] of personAvatarMap) {
        if (av.group.visible === false && id !== myPersonId) continue;   // 오프라인/숨김 제외(본인은 fpv 숨김이어도 포함)
        out.push({
            id, name: av.displayName || id, color: av.shirtColorHex || '#4A90D9',
            x: av.group.position.x, y: av.group.position.y, z: av.group.position.z,
            ry: av.group.rotation.y, isMe: id === myPersonId,
            bubble: av.bubbleEl ? av.bubbleEl.textContent : null,
        });
    }
    return out;
}

// ---- 2D 탑다운 뷰(게더타운식, js/map2d.js) — 3D ↔ 2D 전환 ----
// 배경은 정렬 레이아웃 벡터 도면(건물별 방+기능 라벨), 이동은 기존 WS 위치 동기화를 그대로 탄다.
const map2d = initMap2D({
    getAvatars: getAvatarList,
    floorLabel: (y) => {   // 상층 아바타에 층 배지(2F~4F) — floorCenters(엘리베이터 도착 y)와 최근접 매칭
        if (y <= 1.5) return null;
        let best = null, bd = Infinity;
        for (const n of Object.keys(floorCenters)) {
            const d = Math.abs(floorCenters[n].y - y);
            if (d < bd) { bd = d; best = n; }
        }
        return best && Number(best) > 1 ? `${best}F` : null;
    },
    hideOverlays: () => {          // 3D용 DOM 오버레이(라벨·말풍선·버튼) 상시 숨김 — 새로 생성되는 것 포함
        for (const [, av] of personAvatarMap) {
            if (av.labelEl) av.labelEl.style.visibility = 'hidden';
            if (av.btnsEl) av.btnsEl.style.display = 'none';
            if (av.bubbleEl) av.bubbleEl.style.visibility = 'hidden';
            if (av.meetingEl) av.meetingEl.style.visibility = 'hidden';
        }
    },
    moveMy: (nx, nz, ry) => {
        const av = myPersonId && personAvatarMap.get(myPersonId);
        if (!av) return false;
        if (sittingSeat) standUp();
        av.group.position.set(nx, PERSON_GROUND_Y, nz);   // 2D는 지상 평면 기준(상층에 있었다면 1층으로)
        av.group.rotation.y = ry;
        av.targetPos = { x: nx, z: nz };                  // 원격 보간이 위치를 되돌리지 않게 목표점 동기화
        _throttleSendPosition();
        return true;
    },
    persistMy: () => { if (myPersonId) persistPersonPosition(myPersonId); },
    showToast: (msg) => showSelectToast(msg),
    openMealPlan: () => { if (window.__showMealPlan) window.__showMealPlan(); },
    focus3D: (v) => { _exitToTower(); tweenView(v, 900); },   // 2D→3D 건물 입장 시 해당 건물 조망
    getSprite: (id) => getAvatarSprite(id),                   // 실제 캐릭터 스프라이트(정면 렌더 캐시)
    onEnter: () => {
        if (inJumpmap) exitJumpmap();              // 점프맵 좌표계와 분리(스냅샷 전에 오피스 복원)
        keysDown.clear(); pressedMoveKeys.clear();
        renderer.domElement.style.display = 'none';
        if (holoRenderer) holoRenderer.domElement.style.display = 'none';
    },
    onExit: () => {
        renderer.domElement.style.display = '';
        if (holoRenderer) holoRenderer.domElement.style.display = '';
    },
});
// 뷰 스위처에 2D/3D 토글 버튼 추가 + 마지막 모드 복원
(() => {
    const wrap = document.getElementById('view-switcher');
    if (!wrap) return;
    const b = document.createElement('button');
    b.title = '게더타운식 2D 탑다운 뷰 ↔ 3D 전환';
    b.style.cssText = `
        background:rgba(255,255,255,0.08); color:#a8ffd8; border:none; border-radius:7px;
        padding:6px 11px; font-family:monospace; font-size:12px; cursor:pointer; white-space:nowrap;
        margin-left:4px; border-left:1px solid #2a3550;`;
    const sync = () => { b.textContent = map2d.isActive() ? '🏙 3D' : '🗺 2D'; };
    map2d.onChange(sync);
    b.onclick = () => map2d.toggle();
    sync();
    wrap.appendChild(b);
    if (localStorage.getItem('tp_view2d') === 'on') setTimeout(() => map2d.enter(), 700);   // 마지막 모드 복원(씬 안정화 후)
})();
...
window.addEventListener('keydown', (e) => {
    ...
    if (_isTyping()) return;
    if (window.__map2dActive) return;   // 2D 모드: 이동·점프·뷰 단축키는 map2d가 전담
    if (window.__tetrisActive) return;  // 테트리스 오버레이 중: 게임이 키를 전담
    // SPACE: 포탈 반경 안이면 점프맵 입장/복귀, 그 외엔 본인 아바타 점프(지상에서만 → 가구 위 착지 가능)
    if (e.code === 'Space') {
        e.preventDefault();
        if (e.repeat) return;            // OS 자동반복 무시 — 꾹 누를 때 착지 즉시 재점프·포탈 재발동 방지
...
function enterJumpmap() {
    ...
    tetrisTower.group.visible = false;
    if (iceCreamGroup) iceCreamGroup.visible = false;   // 베스킨라빈스도 함께(루트 직속 그룹)
    try { _hideOfficeAvatars(); } catch (e) { console.error('[jumpmap] 아바타 숨김 오류', e); }
    // 점프맵 표시 + 아바타 텔레포트 — 텔레포트·카메라를 먼저 완결하고,
    // 리셋류(resetWax)는 try로 격리해 어떤 예외에도 "입장 자체"는 성공하게 한다.
    ...
    av.targetPos = { x: sp.x, z: sp.z };            // 원격 보간·에코가 되돌리지 않게 목표점 동기화
    _airborne = false; _jumpVelY = 0; _coyoteUntil = 0; _jumpBufferedUntil = 0;
    ...
    try {
        ...
        console.error('[jumpmap] resetWax 오류', e);
        showSelectToast('⚠ 왁스 초기화 오류: ' + ((e && e.message) || e));
    }
...
function exitJumpmap() {
    ...
    tetrisTower.group.visible = true;
    ...
    if (av && _savedOffice) {
        ...
        av.targetPos = { x: _savedOffice.x, z: _savedOffice.z };   // 복귀 좌표도 보간 목표 동기화
    }
    _airborne = false; _jumpVelY = 0; _coyoteUntil = 0; _jumpBufferedUntil = 0;
    keysDown.clear();
...
/** 매 프레임: 본인 아바타가 활성 포탈 입장 반경 안인지 갱신 + 힌트 표시. 실제 발동은 SPACE(trySelfJump 앞)에서. */
function updatePortals() {
    const av = myPersonId && personAvatarMap.get(myPersonId);
    if (!av) { _portalInRange = false; _hidePortalHint(); return; }
    // 오피스에는 포탈이 2개(왁뿌=맵 전환, 테트리스=오버레이 게임) — 반경 안 첫 포탈이 활성
    const portals = inJumpmap
        ? [[jumpMapArea.returnPortal, '🏢 SPACE로 오피스 복귀', 'exit']]
        : [[jumpTower.portal, '🌀 SPACE로 점프맵 입장', 'jumpmap'],
           [tetrisTower.portal, '🧱 SPACE로 테트리스 시작', 'tetris']];
    let hit = null;
    for (const entry of portals) {
        const p = entry[0];
        const dx = av.group.position.x - p.x, dz = av.group.position.z - p.z;
        const rangeR = p.r + 2.2;                   // 입장 가능 반경(= 힌트 반경)
        if (dx * dx + dz * dz <= rangeR * rangeR) { hit = entry; break; }
    }
    if (!hit) {                                     // 모든 포탈 반경 이탈 → 재장전 + 힌트 숨김
        _portalInRange = false; _portalArmed = true; _portalAction = null; _hidePortalHint();
        return;
    }
    _portalInRange = true;
    _portalAction = hit[2];
    // 발동 직후(재장전 전)엔 힌트를 숨겨 SPACE=점프로 쓰게 하고, 반경을 벗어났다 다시 오면 힌트 노출
    if (_portalArmed) _showPortalHint(hit[1]);
    else _hidePortalHint();
}

/** SPACE가 포탈 발동에 쓰일 상황인지(포탈 반경 안 + 재장전 상태). true면 동작 실행. */
function tryPortalToggle() {
    if (!(_portalInRange && _portalArmed)) return false;
    _portalArmed = false;
    _hidePortalHint();
    if (_portalAction === 'tetris') {
        keysDown.clear();                           // 잔여 이동키 정리 후 오버레이 오픈
        const acct = getAccount() || {};            // 로그인 사용자면 서버 순위(TETRIS-RANK)에 점수 제출
        openTetris({
            player: acct.username ? { email: acct.username, name: acct.name || acct.username } : null,
            onClose: (score) => {
                showSelectToast(`🧱 테트리스 종료 — ${score.toLocaleString()}점`);
                refreshTetrisRanking();             // 타워 순위판 갱신(내 기록 반영)
            },
        });
    } else if (inJumpmap) exitJumpmap();
    else enterJumpmap();
    return true;
}
...
function _onJumpmapLanded(mesh) {
    if (mesh && mesh.userData.keycapIdx !== undefined) {   // 키캡 길 — 클릭음 + RGB 백라이트
        onKeycapStep(mesh);
        return;
    }
    if (mesh && mesh.userData.puddingIdx !== undefined) {  // 퐁실 푸딩 — 보잉음 + 젤리 출렁
        onPuddingStep(mesh, true);
        return;
    }
...
// ============================================
// 개발 편의: dev-reload 후 작업 위치로 자동 이동
// ------------------------------------------------------------------
// POST /api/dev/reload { goto } → WS 'dev-reload' 수신부가 goto를 sessionStorage에
// 저장하고 새로고침한다. 여기서는 로드 후 본인 아바타·맵 준비를 기다렸다가
// 맵 전환(office/jumpmap) + 텔레포트 + 카메라 이동을 적용한다.
// goto = { map:'office'|'jumpmap', x, z, y?, view?:{ pos:[x,y,z], target:[x,y,z] } }
//   (jumpmap이면 좌표·뷰는 점프맵 로컬 기준 — JUMPMAP_ORIGIN 오프셋을 여기서 더함)
// ============================================
(function _applyDevGoto() {
    let goto = null;
    try { goto = JSON.parse(sessionStorage.getItem('dev.goto') || 'null'); } catch { /* noop */ }
    if (!goto) return;
    const t0 = Date.now();
    const timer = setInterval(() => {
        if (Date.now() - t0 > 30000) {                 // 30초 내 준비 안 되면(미로그인 등) 포기
            clearInterval(timer);
            try { sessionStorage.removeItem('dev.goto'); } catch { /* noop */ }
            return;
        }
        const av = myPersonId && personAvatarMap.get(myPersonId);
        if (!av || !officeJumpTargets) return;         // 로그인 복원·착지면 수집 대기
        clearInterval(timer);
        try { sessionStorage.removeItem('dev.goto'); } catch { /* noop */ }
        if (window.__map2dActive && window.__map2d) window.__map2d.exit();   // goto는 3D 기준
        const toJump = goto.map === 'jumpmap';
        if (toJump && !inJumpmap) enterJumpmap();
        else if (!toJump && inJumpmap) exitJumpmap();
        const ox = toJump ? JUMPMAP_ORIGIN.x : 0, oz = toJump ? JUMPMAP_ORIGIN.z : 0;
        if (typeof goto.x === 'number' && typeof goto.z === 'number') {
            av.group.position.set(ox + goto.x, typeof goto.y === 'number' ? goto.y : 0, oz + goto.z);
            _airborne = false; _jumpVelY = 0; _coyoteUntil = 0; _jumpBufferedUntil = 0;
        }
        if (goto.view && goto.view.pos && goto.view.target) {
            tweenView({
                pos: [ox + goto.view.pos[0], goto.view.pos[1], oz + goto.view.pos[2]],
                target: [ox + goto.view.target[0], goto.view.target[1], oz + goto.view.target[2]],
            }, 800);
        }
        showSelectToast('🛠 작업 위치로 이동했습니다');
    }, 400);
})();
