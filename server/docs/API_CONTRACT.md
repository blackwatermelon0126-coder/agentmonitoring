...
7. [인앱 Teams 채팅 API](#7-인앱-teams-채팅-api)
...
METAOFFICE 3D 오피스 안에서 Microsoft Teams 채팅을 조회·전송한다(WJ CHAT-01).
구현: `server/teams/chatService.js`(Graph 래핑) + `server/server.js`(라우트) + `three3d/js/chat-panel.js`(UI).

> **공통 규약**
> - 토큰: `refreshTokenIfNeeded()` 로 확보. 토큰 없으면 `401 { "error": "not_authenticated" }`.
> - Graph 오류는 원 상태코드 그대로 전파 (`{ "error": "graph_error", "status", "reason" }`).
> - 조회(GET)는 제한 없음, **전송(POST)은 외부 비가역 호출이라 `requireLoopback`** (루프백 외 주소 403).
> - 필요 권한: `Chat.Read`·`Chat.ReadBasic`(목록·읽기), `Chat.ReadWrite`(전송). scope 변경 시 재로그인 필요.

### 7-1. GET /api/chats

내 채팅방 목록을 최근 갱신순으로 반환한다. Graph: `GET /me/chats?$expand=members&$top=50`.

응답 `200` — 배열:

| 필드 | 타입 | 설명 |
|------|------|------|
| `chatId` | `string` | 채팅 식별자 (`19:...@thread.v2`) |
| `title` | `string` | `topic`(그룹) → 나 제외 멤버 displayName 조인(1:1) → `(제목 없음)` |
| `chatType` | `string` | `oneOnOne` / `group` / `meeting` |
| `memberNames` | `string[]` | 참여자 displayName 목록 |
| `lastMessageAt` | `string \| null` | 마지막 갱신 시각(ISO). 정렬 기준(없으면 뒤로) |

### 7-2. GET /api/chats/:chatId/messages

특정 채팅의 최근 메시지를 **오래된→최신** 순으로 반환한다. Graph: `GET /chats/{chatId}/messages?$top={limit}&$orderby=createdDateTime desc`.

쿼리: `limit` (기본 20, 1~50 클램프). 시스템 메시지(`messageType!=='message'`)·본문 없는 메시지 제외.
...
|------|------|------|
| `id` | `string` | 메시지 id |
| `text` | `string` | 평문 (`plainTextContent` 우선, 없으면 HTML 태그 제거) |
| `senderName` | `string` | 발신자 displayName |
| `senderId` | `string` | 발신자 AAD object id (소문자) |
| `createdDateTime` | `string` | 작성 시각(ISO) |
| `isMine` | `boolean` | 발신자 id == 내 계정 id(`localAccountId`) |

### 7-3. POST /api/chats/:chatId/messages  *(requireLoopback)*

채팅에 텍스트 메시지를 전송한다. Graph: `POST /chats/{chatId}/messages` body `{ body: { contentType: 'text', content } }`.

요청 JSON:

| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `text` | `string` | Y | 전송할 평문. trim 후 비면 `400 { "error": "empty_message" }`. 4000자 초과 시 `400 { "error": "message_too_long" }` |

응답 `201` — 전송된 메시지(7-2 요소 구조, `isMine: true`).

> **실시간 수신**: 15초 서버 폴링이 신규 메시지를 `teams-notification` WS 로 broadcast(§는 pollOnce). 프론트는 열린 채팅창을 8초 폴링 + 이 WS로 갱신한다.
